...
import { getCourseById, isUserEnrolled } from "@/lib/moodle";
...
const CourseDetails = async ({
  ...
  // NOTE: Dans une version finale, on stockerait l'ID Moodle de l'utilisateur en cookie
  const isEnrolled = username ? await isUserEnrolled(1, courseId) : false; // Placeholder ID 1
                ...
                <div className="rounded-3xl bg-primary px-6 pt-7 pb-8 shadow-card">
                  ...
                  {isEnrolled ? (
                    <Button
                      disabled
                      size="lg"
                      className="mt-8 w-full py-1.5 px-6 opacity-70 bg-green-100 text-green-700"
                    >
                      <span className="w-full text-center">Already Enrolled</span>
                    </Button>
                  ) : (
                    <form action={async () => {
                      'use server';
