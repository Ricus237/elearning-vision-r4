...
const StudentSidebar = () => {
  ...
  return (
    <>
      {/* Mobile Top Navbar */}
      <div className="md:hidden sticky top-0 z-50 bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between shadow-sm">
        ...
        <button 
          ...
          className="p-2 bg-gray-50 rounded-xl text-gray-600 hover:text-purple-600 transition-colors"
        >
      ...
      {/* Overlay */}
      {isOpen && (
        <div onClick={() => setIsOpen(false)} className="fixed inset-0 bg-gray-900/40 backdrop-blur-sm z-30 md:hidden" />
      )}

      {/* Sidebar Content */}
      <aside className={`
        fixed inset-y-0 left-0 z-40 bg-white border-r border-gray-100 px-6 py-8 flex flex-col shrink-0 md:w-72 md:sticky md:top-0 md:h-screen w-[280px] shadow-2xl md:shadow-none transition-transform duration-300 transform md:translate-x-0
        ${isOpen ? "translate-x-0" : "-translate-x-full"}
      `}>
        <div className="flex items-center gap-4 px-2 py-6 mb-10 border-b-2 border-gray-50 hidden md:flex">
